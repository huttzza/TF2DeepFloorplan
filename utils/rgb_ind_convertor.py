...
def rgb2ind(im, color_map=ROOM_TYPE):#floorplan_room_map):
...
def ind2rgb(ind_im, color_map=ROOM_TYPE):#floorplan_room_map):
	rgb_im = np.zeros((ind_im.shape[0], ind_im.shape[1], 3))
	
	ind_im_sqz = np.zeros((ind_im.shape[0], ind_im.shape[1]))
	'''
	for i, x in enumerate(ind_im):
		#print(x)
		for j, y in enumerate(x):
			#ind_im_sqz[i][j] = np.argmax(y)#find_true(y) #argmax 값이 자꾸 바뀜
			rgb_im[i][j] = color_map[np.argmax(y)]
	'''
	for i, rgb in color_map.items():
		rgb_im[(ind_im==i)] = rgb
